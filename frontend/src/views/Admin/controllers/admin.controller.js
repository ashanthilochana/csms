...
// Add a new client
AdminController.addClient = async (nic, email, name, address, contactNumber, branchId) =>{
    ...
    if (nic === "") {
        ...
        let reqBody = {
            ...
            try{
                let response = await AdminService.addClient(reqBody);
                if(response.error){
...
// Add a new order
AdminController.addOrder = async (weight, sendingDate, paymentDate,packageTypes,sendingBranch,receivingBranch,speicialNotes,orderStatus,sender,receiver,contactNumber,address) => {
    ...
    if (weight === "") {
        ...
        let reqBody = {
            ...
            try{
                let response = await AdminService.addOrder(reqBody);
                if(response.error){
                    return {error : response.error};
                }
                return {message : "Success Message"};
...
// AdminController.addTransportagent = async (nic, email, name, vehicleNumber, contactNumber, branchId) =>{
